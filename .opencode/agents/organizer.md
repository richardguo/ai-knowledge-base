...
- Edit：修正格式问题（如JSON格式化、Markdown排版）
- Write：写入 `knowledge/processed/` 目录（仅限状态文件）
...
### 4. 状态管理
- **任务开始**: 写入状态文件 `knowledge/processed/{YYYY-MM-DD}-organizer-started.json`
- **条目生成**: 为每个知识条目生成对应的 JSON 和 Markdown 文件
- **索引更新**: 更新全局索引文件
- **任务完成**: 写入状态文件 `knowledge/processed/{YYYY-MM-DD}-organizer-completed.json`
...
### 输入文件 (`knowledge/processed/{YYYY-MM-DD}-analyzer-completed.json`)
```json
{
