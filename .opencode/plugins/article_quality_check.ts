...
const QUALITY_SCRIPT = "hooks/check_quality.py"
...
async function runQualityCheck(
  ...
  if (files.length === 0) {
    await client.app.log({
      body: {
        service: "article_quality_check",
        level: "info",
        message: "未找到当日生成的 JSON 文件，跳过质量检查",
      },
    })
    return
  }
  ...
  await client.app.log({
    body: {
      service: "article_quality_check",
      level: "info",
      message: `开始质量检查 ${files.length} 个当日 JSON 文件`,
    },
  })

  const result = await $`python ${QUALITY_SCRIPT} ${files.join(" ")}`.quiet()
  ...
  if (result.exitCode !== 0) {
    ...
    await client.app.log({
      body: {
        service: "article_quality_check",
        level: "error",
        message: `质量检查失败，存在等级为 C 的文件`,
        extra: { output },
      },
    })

    throw new Error(
      `[article_quality_check] 质量检查失败。\n` +
        `检测到等级为 C 的文件，请重新处理。\n\n` +
        `${output}`
    )
  }
  ...
  await client.app.log({
    body: {
      service: "article_quality_check",
      level: "info",
      message: `质量检查通过: ${files.length} 个文件全部合格`,
    },
  })
}

export const ArticleQualityCheck: Plugin = async ({ client, $ }) => {
  return {
    "tool.execute.after": async (input, output) => {
      ...
      if (!statusFile) {
        await client.app.log({
          body: {
            service: "article_quality_check",
            level: "info",
            message: "未找到 organizer 状态文件，跳过质量检查",
          },
        })
        return
      }
      ...
      if (!statusData || statusData.status !== "completed") {
        await client.app.log({
          body: {
            service: "article_quality_check",
            level: "info",
            message: `organizer 状态为 "${statusData?.status || "unknown"}"，跳过质量检查`,
          },
        })
        return
      }
      ...
      await client.app.log({
        body: {
          service: "article_quality_check",
          level: "info",
          message: `organizer 已完成 (创建了 ${statusData.entries_created} 个条目)，触发质量检查`,
        },
      })

      await runQualityCheck(client, $)
    },
  }
}
