...
系统 SHALL 在 `openspec/specs/schemas/collector-output.json` 定义 Collector 产出数据的 JSON Schema，覆盖 `knowledge/raw/*.json` 的结构。Schema SHALL 要求以下顶层字段：
- `collected_at`：字符串，ISO 8601 格式，必填
- `source`：字符串，枚举值为 `github-search` 或 `github-trending`，必填
- `version`：字符串，必填
- `items`：数组，必填
...
- `language`：字符串，必填（可为空字符串）
- `popularity`：整数，必填，最小值 0
- `popularity_type`：字符串，必填
- `topics`：字符串数组，必填
- `readme`：字符串，必填（可为空字符串）
- `summary`：字符串，必填（可为空字符串，Collector 阶段置空）
- `created_at`：字符串，必填
- `updated_at`：字符串，必填
- `collected_at`：字符串，必填

`source` 为 `github-trending` 时，SHALL 额外要求 `since` 字段（字符串，必填）。
...
#### Scenario: Valid collector output with github-trending source
- **WHEN** 校验一个包含 `source: "github-trending"` 且含 `since` 字段的 collector output JSON
- **THEN** schema 校验通过，不报告错误
...
#### Scenario: Invalid source enum value
- **WHEN** 校验一个 `source` 值为 `"hackernews"` 的 collector output JSON
- **THEN** schema 校验失败，报告 `source` 不在枚举范围内
...
系统 SHALL 在 `openspec/specs/schemas/knowledge-article.json` 定义知识条目的 JSON Schema，覆盖 `knowledge/articles/*.json` 的结构。Schema SHALL 要求以下字段：
- `id`：字符串，必填
- `title`：字符串，必填
- `source`：字符串，必填
- `url`：字符串，URI 格式，必填
...
- `tags`：字符串数组，必填
- `relevance_score`：整数，必填，范围 1-10
- `metadata`：对象，必填，包含 `author`、`language`、`popularity`、`popularity_type`、`topics`、`created_at`、`updated_at`、`category`、`maturity` 字段
...
系统 SHALL 在 `openspec/specs/schemas/index.json` 定义索引文件的 JSON Schema，覆盖 `knowledge/articles/index.json` 的结构。Schema SHALL 定义为对象数组，每个对象 SHALL 要求：
- `id`：字符串，必填
- `title`：字符串，必填
- `url`：字符串，URI 格式，必填
- `collected_at`：字符串，必填
